        ...
        if (users && users.length>0)
        {
           var userId = users[0].Id;
            ...
            .then(data=> {
                ...
                let fields = [];
               if(!casereturnInfo.Success) 
                { fields.push({title:"CaseID "+ casereturnInfo.oldCaseInfo.CaseNum+" has already been claimed by: ", value:casereturnInfo.oldCaseOwner.Name, short:false});}
              else
                {fields.push({title: "CaseID "+casereturnInfo.oldCaseInfo.CaseNum +" has now been claimed by "+casereturnInfo.requestSFUser.Name, value:"", short:false});};
